-- Fix type mismatch in create_group_master_folio: group_bookings.group_id is text, p_group_id is uuid
-- Cast p_group_id to text everywhere we compare or insert into group_bookings

CREATE OR REPLACE FUNCTION public.create_group_master_folio(p_tenant_id uuid, p_group_id uuid, p_master_booking_id uuid, p_guest_id uuid, p_group_name text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM group_bookings
  WHERE group_id = p_group_id::text
  ...
  INSERT INTO finance_audit_events (
    ...
    jsonb_build_object(
      ...
      'version', 'GROUP-MASTER-FOUNDATION-V1-FIX-TYPES'
    )
  );
