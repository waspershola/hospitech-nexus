-- Migration: Fix folio_post_charge function overloading
-- Marker: FIX-FOLIO-CHARGE-OVERLOAD-V1
-- Root Cause: Multiple function signatures causing PGRST203 error preventing extend-stay from working

-- Drop ALL possible overloaded signatures to ensure clean state
DROP FUNCTION IF EXISTS public.folio_post_charge(UUID, NUMERIC, TEXT, TEXT, UUID, TEXT);
DROP FUNCTION IF EXISTS public.folio_post_charge(TEXT, NUMERIC, TEXT, TEXT, UUID, TEXT);
DROP FUNCTION IF EXISTS public.folio_post_charge(TEXT, NUMERIC, TEXT, TEXT, UUID, TEXT, UUID, TEXT);
DROP FUNCTION IF EXISTS public.folio_post_charge(UUID, NUMERIC, TEXT);

-- Recreate the single canonical 8-parameter version with defensive UUID handling
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RETURN jsonb_build_object(
        ...
        'version', 'FIX-FOLIO-CHARGE-OVERLOAD-V1'
      );
    END;
  END;
